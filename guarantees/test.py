...
class TEST4(BaseTestCase):
    ...
    def runTest(self):
        ...
        messages_req = []            # список отправленных сообщений

        for i in range(num_messages):
            sender_req = Message('INFO-4', str(i))

            messages_req.append(sender_req)           # добавляем в список отправленных сообщений
        ...
        for i in range(len(messages)):
            ...
            self.assertEqual(messages[i].body, messages_req[i].body)    # проверка сохранения порядка
...
class RandomTEST4(BaseTestCase):

    def runTest(self):
        self.assertTrue(self.ts.wait_processes(2, 1), "Startup timeout")
        self.ts.set_real_time_mode(False)

        client_req1 = Message('INFO-4', 'some message1')
        client_req2 = Message('INFO-4', 'some message2')
        client_req3 = Message('INFO-4', 'some message3')
        messages = [client_req1, client_req2, client_req3]   # добавляем в список отправленных сообщений

        # Send 5 identical messages

        for i in range(len(messages)):                              # по очереди отправляем сообщения
            self.ts.send_local_message('sender', messages[i])
            self.ts.send_local_message('sender', messages[i])
            self.ts.send_local_message('sender', messages[i])
            self.ts.send_local_message('sender', messages[i])
            self.ts.send_local_message('sender', messages[i])
        ...
        for i in range(len(messages)):                                         # проверка сохранения порядка
            sender_resp = self.ts.step_until_local_message('receiver', 1)
            self.assertIsNotNone(sender_resp, "Receiver response timeout")
            self.assertEqual(sender_resp.type, 'INFO-4')
            self.assertEqual(sender_resp.body, messages[i].body)
